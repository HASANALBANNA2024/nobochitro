...
class CancelListItem extends StatelessWidget {
  ...
  void _showRefundDialog(BuildContext context, String status) {
    TextEditingController transCtrl = TextEditingController(
      text: item['refund_transaction'] ?? "",
    );

    /// web and mobile view variable
    Uint8List? webImage;
    File? mobileFile;
  ...
  Widget build(BuildContext context) {
    ...
    String current =
        (item['refund_status']?.toString().toLowerCase()) ?? 'request';
      ...
      child: Container(
        ...
        child: Card(
          ...
          child: ExpansionTile(
            ...
            subtitle: Text(
              "Status: ${item['booking_status']?.toUpperCase() ?? ''} | Refund Amount: ${item['refund_amount'] ?? 0}",
            ),
            children: [
              Padding(
                          ...
                          child: ElevatedButton(
                            onPressed: () async {
                              ...
                              double paymentAmount =
                                  double.tryParse(
                                    item['payment_amount']?.toString() ?? "0",
                                  ) ??
                                  0.0;
                    ...
                    Wrap(
                      ...
                      children: [
                        if (item['transaction_image_url'] != null &&
                            item['transaction_image_url'].toString().isNotEmpty)
                          _buildImagePreview(
                            context,
                          ...
                          onSelected: (i < cur)
                              ...
                              : (_) async {
                                  if (s == 'refunded') {
                                    ...
                                    await _controller.updateRefundStatusOnly(
                                      item['booking_id'],
                                      s,
                                      onUpdate,
                                    );
